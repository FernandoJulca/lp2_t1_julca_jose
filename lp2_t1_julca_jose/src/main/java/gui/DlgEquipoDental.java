...
import model.DentistaJoseJC;
import model.EquipoDentalJoseJC;
import model.EspecialidadJoseJC;
...
public class DlgEquipoDental extends JDialog implements ActionListener {
	...
	void cargarDentistas() {
		...
		try {
			String jpql = "select d from DentistaJoseJC d";
			List<DentistaJoseJC> lstDentista = manager.createQuery(jpql, DentistaJoseJC.class).getResultList();
			
			for(DentistaJoseJC dentista : lstDentista) {
	...
	void listar() {
		...
		EntityManager manager = JPAUtil.getEntityManager();
		String jpql = "SELECT e FROM EquipoDentalJoseJC e JOIN e.idDentista d JOIN d.idEspecialidad";
		
		try {
			
			List<EquipoDentalJoseJC> resultado = manager.createQuery(jpql, EquipoDentalJoseJC.class).getResultList();

			for (EquipoDentalJoseJC equipo : resultado) {
			    DentistaJoseJC dentista = equipo.getIdDentista();
			    EspecialidadJoseJC especialidad = dentista.getIdEspecialidad();

			    imprimir("Nro Equipo................: " + equipo.getNroEquipo());
			    imprimir("Nombre....................: " + equipo.getNombre());
			    imprimir("Costo.....................: " + equipo.getCosto());
			    imprimir("Fecha de adquisicion......: " + equipo.getFecAdquisicion());
			    imprimir("Dentista..................: " + dentista.getNombre());
	...
	void adicionar() {
		int equipo = Integer.parseInt(txtNroEquipo.getText());
		String nombre = txtNombre.getText();
		double costo = Double.parseDouble(txtCosto.getText());
		
		int estado = (Integer) cboEstados.getSelectedItem();
		DentistaJoseJC dentista = (DentistaJoseJC) cboDentistas.getSelectedItem();
		
		 EquipoDentalJoseJC equipoDentista = new EquipoDentalJoseJC();
	}
